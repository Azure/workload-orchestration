...
- Download JSON/YAML files for schemas, configs from the shared team drive.
...
> **Note:** All three script variants (PowerShell `.ps1`, Python `.py`, Bash `.sh`) are functionally equivalent. Choose whichever matches your environment.
...
**PowerShell:**
```powershell
.\powershell\infra_onboarding.ps1 mock-data.json -skipAzExtensions $True -skipCustomLocationCreation $True
```

**Python:**
```bash
python python/infra_onboarding.py mock-data.json --skip-az-extensions --skip-custom-location-creation
```

**Bash:**
```bash
bash shell/infra_onboarding.sh mock-data.json --skip-az-extensions --skip-custom-location-creation
```
